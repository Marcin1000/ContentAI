...
| **Unverifiable filler** | Asked to write past the end of its sources, a model reaches into training memory, which has no date and no address and therefore cannot be checked. Three outcomes are allowed instead: the knowledge base has it, a web search finds it and cites the page, or the article does not claim it. |
...
node serwer/testy.js                   # server logic, 282 tests
```
